...
import { collection, query, where, onSnapshot, getDocs, addDoc, serverTimestamp, doc, getDoc, setDoc, writeBatch } from 'firebase/firestore';
import { db, handleFirestoreError, OperationType } from './firebase';
import { useAuth } from './AuthContext';
import { useApp } from './App';
import { Calendar as CalendarIcon, CheckCircle2, AlertTriangle, Save, Printer, Trash2 } from 'lucide-react';
...
export const DailyReport = ({ date: propDate, viewOnly = false }: { date?: string, viewOnly?: boolean }) => {
  ...
  const calculateTotals = () => {
    ...
    todayTransactions.forEach(tx => {
      ...
      if (tx.type === 'Receive' || tx.type === 'Cash Receive') receive += tx.amount || 0;
      else if (tx.type === 'Payment' || tx.type === 'Cash Payment') payment += tx.amount || 0;
  ...
  const handleSave = async () => {
    ...
    if (!todayClosing) {
      setErrorModal("Cannot save report: Cash Closing has not been done for today.");
      return;
    }
  ...
  const handleDelete = async () => {
    if (!window.confirm("Are you sure you want to delete this saved daily report?")) return;
    
    setLoading(true);
    try {
      const q = query(collection(db, 'daily_summaries'), where('date', '==', date));
      const snap = await getDocs(q);
      const batch = writeBatch(db);
      snap.forEach(d => batch.delete(d.ref));
      await batch.commit();
      setSuccessModal("Report deleted successfully.");
      // Explicitly reset states to reflect deletion immediately
      setIsAlreadySaved(false);
      if (viewOnly) setDate(date); // Trigger re-render if needed
    } catch (error) {
      console.error("Error deleting report:", error);
      setErrorModal("Error deleting report.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6 pb-20">
      {(viewOnly && !isAlreadySaved) ? (
        <div className="bg-white p-10 rounded-[2.5rem] border border-slate-100 shadow-sm text-center">
          <p className="text-slate-500 font-bold">No saved report found for this date.</p>
        </div>
      ) : (
        <div className="space-y-8">
          {!propDate && !viewOnly && (
          <div className="flex justify-between items-center bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm no-print">
          ...
          <div className="max-w-md mx-auto space-y-6">
            <div className="text-center space-y-1 pb-4 border-b border-slate-100">
              <h1 className="text-xl sm:text-2xl font-black leading-tight tracking-tight animate-color-run">
            ...
            <h3 className="text-lg font-black text-slate-800 pt-2 uppercase tracking-widest">{t('dailyReport')}</h3>
          ...
          {!propDate && !viewOnly && (
            <div className="mt-4 no-print">
              <button
                onClick={handleSave}
                disabled={loading || isAlreadySaved || !todayClosing}
                className={cn(
                  ...
                  (isAlreadySaved || !todayClosing) ? "bg-slate-300 cursor-not-allowed text-slate-500" : "bg-slate-900"
                )}
              >
          ...
          {viewOnly && isAlreadySaved && role === 'super_admin' && (
            <div className="mt-6 no-print">
              <button
                onClick={handleDelete}
                disabled={loading}
                className="w-full py-4 bg-rose-50 text-rose-600 border-2 border-rose-100 font-black rounded-2xl hover:bg-rose-100 transition-all flex items-center justify-center gap-2"
              >
                {loading ? <div className="w-5 h-5 border-2 border-rose-300 border-t-rose-600 rounded-full animate-spin" /> : <Trash2 size={20} />}
                Delete Saved Report
              </button>
            </div>
          )}
