...
import { formatCurrency, toBengaliNumber, cn, getDirectDriveUrl, formatNumberWithCommas, parseNumberFromCommas } from './lib/utils';
...
export const TransactionMenu = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const f = parseFloat(parseNumberFromCommas(fine)) || 0;
    const d = parseFloat(parseNumberFromCommas(discount)) || 0;
                      ...
                      <input 
                        ...
                        onChange={e => {
                          ...
                          if (raw === '' || !isNaN(Number(raw))) {
                            setAmount(formatNumberWithCommas(raw, language));
                          }
                        }}
                      />
                    </div>
                    ...
                    <div className="space-y-1">
                      ...
                      <input 
                        ...
                        onChange={e => {
                          const raw = parseNumberFromCommas(e.target.value);
                          if (raw === '' || !isNaN(Number(raw))) {
                            setFine(formatNumberWithCommas(raw, language));
                          }
                        }}
                        placeholder="0"
                      />
                      ...
                      <div className="space-y-1">
                        ...
                        <input 
                          ...
                          onChange={e => {
                            const raw = parseNumberFromCommas(e.target.value);
                            if (raw === '' || !isNaN(Number(raw))) {
                              setDiscount(formatNumberWithCommas(raw, language));
                            }
                          }}
                          placeholder="0"
                        />
                    ...
                    {activeType === 'installment' && (
                      <div className="space-y-1">
                        ...
                        <div className="w-full px-4 py-3 bg-emerald-50 border-2 border-emerald-100 rounded-xl font-black text-xl text-center text-emerald-700">
                          {formatCurrency((parseFloat(parseNumberFromCommas(amount)) || 0) + (parseFloat(parseNumberFromCommas(fine)) || 0))}
                        </div>
                      </div>
                    )}
                ...
                <div className="bg-emerald-600 p-6 text-white flex justify-between items-center">
                  ...
                  <div className="flex items-center gap-2">
                    <button 
                      ...
                      className="flex items-center gap-2 px-4 py-2 bg-emerald-500 hover:bg-emerald-600 rounded-xl transition-colors font-bold text-sm"
                    >
                      <Printer size={18} />
                      {t('print')}
                    </button>
                  ...
                  >
                    ...
                    <div className="bg-emerald-50 p-6 rounded-2xl flex justify-between items-center border border-emerald-100">
                      ...
                      <span className="text-3xl font-black text-emerald-700">
                        {formatNumberWithCommas(parseFloat(parseNumberFromCommas(lastTransaction.amount)) + (parseFloat(parseNumberFromCommas(lastTransaction.fine)) || 0), language)}
                      </span>
                    </div>
