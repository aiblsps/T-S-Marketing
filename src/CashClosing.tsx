...
import { collection, query, where, getDocs, orderBy, limit, onSnapshot, doc, getDoc, addDoc, deleteDoc, serverTimestamp } from 'firebase/firestore';
import { db, auth, handleFirestoreError, OperationType } from './firebase';
import { useAuth } from './AuthContext';
import { useApp } from './App';
import { useNavigate } from 'react-router-dom';
import { formatCurrency, cn, toBengaliNumber, formatNumberWithCommas, parseNumberFromCommas } from './lib/utils';
import { Calculator, Landmark, Info, AlertCircle, CheckCircle2, History, ArrowUpCircle, ArrowDownCircle, Printer, ArrowLeft } from 'lucide-react';
...
export const CashClosing = () => {
  ...
  const totalInputAmount = (parseFloat(parseNumberFromCommas(todayCash)) || 0) + (parseFloat(parseNumberFromCommas(todayMother)) || 0);
  ...
  const cashDiff = denominationTotal - (parseFloat(parseNumberFromCommas(todayCash)) || 0);
  ...
  const handleCashClosing = async () => {
    ...
    const cash = parseFloat(parseNumberFromCommas(todayCash)) || 0;
    const mother = parseFloat(parseNumberFromCommas(todayMother)) || 0;
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between no-print mb-4">
          ...
          <button 
            ...
            className="flex items-center gap-2 px-6 py-2 bg-emerald-600 text-white rounded-xl font-black hover:bg-emerald-700 active:scale-95 transition-all shadow-lg shadow-emerald-100"
          >
            <Printer size={20} />
            {t('print')}
          </button>
        ...
        <div className="text-center space-y-1 pb-6 border-b-2 border-slate-100">
          <h1 className="text-2xl sm:text-3xl font-black leading-tight tracking-tight animate-color-run">Al-Arafah Islami Bank PLC</h1>
          <h2 className="text-lg font-black animate-color-run">SPS Bazar Outlet</h2>
          <p className="text-xs font-bold uppercase tracking-widest animate-color-run">Kayaria Lanch Ghat, Kayaria, <br /> Kalkini, Madaripur.</p>
          <div className="inline-block mt-4 px-6 py-2 bg-slate-900 text-white rounded-full text-sm font-black uppercase tracking-widest">
            {t('cashClosingPage')}
          </div>
        </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4 no-print">
                <div className="space-y-1">
                  ...
                  <input 
                    ...
                    value={formatNumberWithCommas(todayCash, language)}
                    onChange={e => {
                ...
                <div className="space-y-1">
                  ...
                  <input 
                    ...
                    disabled={isClosed}
                    className="w-full px-4 py-3 bg-white border-2 border-slate-200 rounded-xl focus:border-emerald-500 focus:outline-none font-black text-slate-900"
                    value={formatNumberWithCommas(todayMother, language)}
                    onChange={e => {
                      const raw = parseNumberFromCommas(e.target.value);
                      if (raw === '' || !isNaN(Number(raw))) {
                        setTodayMother(raw);
